...
# Change this in .env if your default location is not India/DEL.
DEFAULT_ORIGIN_IATA = os.getenv("DEFAULT_ORIGIN_IATA", "DEL").strip().upper()
...
def parse_route(query: str):
    ...
    # Direct IATA code route: DEL to NRT, or del to nrt
    codes = [
        code.upper()
        for code in re.findall(r"\b[A-Za-z]{3}\b", q)
    ...
    # Pattern: X to Y, e.g. "Delhi to Tokyo" or "India to Japan"
    match = re.search(
    ...
    # Pattern: destination mentioned before origin, e.g.
    # "Plan a 7 days Japan trip from India"
    if " from " in q_lower:
        left_text, origin_text = q_lower.rsplit(" from ", 1)
        dep_iata = resolve_location_to_iata(origin_text)
        destination_mentions = find_location_mentions(left_text)

        if dep_iata and destination_mentions:
            arr_iata = resolve_location_to_iata(destination_mentions[-1])
            if arr_iata and arr_iata != dep_iata:
                return dep_iata, arr_iata
...
def search_flights(query: str, limit: int = 10):
    ...
    The query can mention a route, city, country, airport, or IATA code, for example:
    "Delhi to Tokyo", "flights from India", or "Japan trip from Delhi".

    Args:
        query: Natural-language flight query used to resolve origin/destination.
        limit: Maximum number of live flight records to return, capped at 100.

    Returns:
        A formatted string containing flight details, or a human-readable error
        message when the API key is missing, the request fails, or no data is found.

    Note:
        AviationStack provides live flight/status data, not ticket prices.
    """
    if not API_KEY:
        return (
    ...
    try:
        ...
        return f"Flight API HTTP error: {e}"
